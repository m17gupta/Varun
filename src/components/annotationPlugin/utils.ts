...
export const getCssSelector = (el: Element): string => {
  ...
  const classes = Array.from(el.classList)
    .filter(c =>
      !['hover', 'active', 'focus', 'selected', 'opacity', 'transition', 'duration', 'ease', 'text-', 'bg-', 'p-', 'm-', 'flex', 'grid', 'w-', 'h-', 'top-', 'left-', 'right-', 'bottom-', 'inset-', 'z-'].some(token => c.includes(token)) && c.length > 2,
    );
  ...
  while (current && current.nodeType === Node.ELEMENT_NODE) {
    ...
    if (current.id) {
      ...
      const currentClasses = Array.from(current.classList)
        .filter(c =>
          !['hover', 'active', 'focus', 'selected', 'opacity', 'transition', 'duration', 'ease', 'text-', 'bg-', 'z-', 'top-', 'left-', 'right-', 'bottom-', 'inset-'].some(token => c.includes(token)),
        );
